...
var ConnectUpdate = require('../models/ConnectUpdate.js');
...
var mongoConnectString = process.env.MONGODB_CONNECT;
...
module.exports = function(app, io) {
	...
	res.render('tryDocuSignDocument.ejs');
	...
	// ////////////////////////////////////////////////////////////////////
	function(next) {
	    var url = config.baseUrl + "/envelopes";
	...
	// ////////////////////////////////////////////////////////////////////
	function(next) {
	    var url = config.baseUrl + "/envelopes/" + envelopeId + "/views/recipient";
	    var method = "POST";
	    var body = JSON.stringify({
	...
	function(next) {
	    ...
	    console.dir("Step 1 - Send envelope");
		...
		'Content-Disposition' : 'file; filename="' + "Try DocuSigning.docx" + '"; documentId=1',
		"body" : fs.readFileSync("resources/Try DocuSigning.docx"),
	    } ];
	...
	function(next) {
	    ...
	    console.dir("Step 2 - Get the Embedded Signing View");
	    var url = config.baseUrl + "/envelopes/" + envelopeId + "/views/recipient";
	    var method = "POST";
	    var body = JSON.stringify({
		"returnUrl" : returnUrl + "?envelopeId=" + envelopeId,
	...
	function(next) {
	    ...
	    var url = config.baseUrl + "/envelopes/" + envelopeId + "/documents/1";
    ...
    app.get('/downloadCertificate', function(req, res) {
	...
	function(next) {
	    ...
	    var url = config.baseUrl + "/envelopes/" + envelopeId + "/documents/certificate";
	...
	req.on('end', function() {
		...
		console.log("Recieved update for EnvelopeID [" + json.DocuSignEnvelopeInformation.EnvelopeStatus[0].EnvelopeID + "], Status [" + json.DocuSignEnvelopeInformation.EnvelopeStatus[0].Status) + "]";
		...
		ConnectUpdate.findOne({
			...
			new ConnectUpdate({
			    _id : json.DocuSignEnvelopeInformation.EnvelopeStatus[0].EnvelopeID,
			    envelopeStatus : json.DocuSignEnvelopeInformation.EnvelopeStatus[0].Status
			}).save();
		    } else {
			connectUpdate.envelopeStatus = json.DocuSignEnvelopeInformation.EnvelopeStatus[0].Status;
			connectUpdate.statusTimestamp = Date.now();
			connectUpdate.save();
		...
		io.sockets.emit("connect_update", {
		    envelopeId : json.DocuSignEnvelopeInformation.EnvelopeStatus[0].EnvelopeID,
		    envelopeStatus : json.DocuSignEnvelopeInformation.EnvelopeStatus[0].Status
		});

		console.log("Message emmited from route");
    ...
    app.get('/connectUpdates', function(req, res) {

	var query = ConnectUpdate.find();
